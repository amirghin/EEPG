    ...
    if (buscar!=""){
        ...
        busqueda.done(function(response){
            ...
            if(object.error){
                ...
                var table = "<tr><td>Id</td><td>Nombre</td><td>Apellido</td><td>Genero</td><td>Fecha Ingreso</td><td>Fecha nacimiento</td> <td>Talla</td><td>Peso Meta</td><td class='hidden'>Circunferencia de la muneca</td><td class='hidden'>Antecedentes personales</td> <td>Padecimientos familiares</td> <td>Nombre usuario</td>  <td>Eliminar</td> <td>Modificar</td></tr> ";
                var tableValues = "";
                $.each(object.pacientes, function(key,value){  // funcion para recorrer el objeto JSON parseado
                    tableValues += "<tr><td>"+value.pacientes_id+"</td><td>"+value.pacientes_nombre+"</td><td>"+value.pacientes_apellidos+"</td><td>"+value.pacientes_genero+"</td> <td>"+value.pacientes_fecha+"</td> <td>"+value.pacientes_fecha_nac+"</td> <td>"+value.pacientes_talla+"</td> <td>"+value.pacientes_peso_meta+"</td> <td>"+value.pacientes_circ_muneca+"</td> <td class='hidden'>"+value.pacientes_ant_personales+"</td> <td class='hidden'>"+value.pacientes_padec_familiares+"</td><td>"+value.usuarios_nombre+"</td><td> <input type='button' value='Eliminar'/> </td> <td> <input type='button' value='Modificar'/> </td><td><input type='hidden' value="+ value.pacientes_id+"></td></tr>";
                });
