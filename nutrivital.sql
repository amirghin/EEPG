...
CREATE TABLE IF NOT EXISTS `nutrivital`.`pacientes` (
  `pacientes_id` INT NOT NULL COMMENT 'Llave primaria para cada paciente',
...
CREATE TABLE IF NOT EXISTS `nutrivital`.`controles` (
  ...
  `pacientes_pacientes_id` INT NOT NULL,
  PRIMARY KEY (`controles_id`, `pacientes_pacientes_id`),
  INDEX `fk_controles_pacientes1_idx` (`pacientes_pacientes_id` ASC),
  CONSTRAINT `fk_controles_pacientes1`
    FOREIGN KEY (`pacientes_pacientes_id`)
...
CREATE TABLE IF NOT EXISTS `nutrivital`.`usuarios` (
  ...
  `pacientes_pacientes_id` INT NOT NULL,
  PRIMARY KEY (`usuarios_nombre`, `roles_roles_id`, `pacientes_pacientes_id`),
  INDEX `fk_usuarios_roles1_idx` (`roles_roles_id` ASC),
  INDEX `fk_usuarios_pacientes1_idx` (`pacientes_pacientes_id` ASC),
